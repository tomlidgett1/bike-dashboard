-- ============================================================
-- Hide variant children in the BASE marketplace view
-- ============================================================
-- public_marketplace_cards already excludes children flagged
-- variant_hidden_from_grid, but several surfaces read the base view
-- marketplace_ready_products directly (marketplace search + store-page
-- fallback in /api/marketplace/products, public store profiles,
-- similar-products). Those leaked the hidden child variants.
--
-- We add the filter to the base view. CREATE OR REPLACE VIEW cannot be used:
-- the view's SELECT * / p.* column list is frozen at creation and no longer
-- lines up with a fresh p.* expansion, so REPLACE fails (42P16). We must DROP
-- and recreate (which has no column-matching constraint). That requires
-- dropping the dependent materialized views first, then recreating them.
-- The get_bike_stores_for_marketplace_filters() SQL function has a string body
-- (no hard dependency), so DROP VIEW is not blocked. The refresh function and
-- pg_cron schedule persist (they resolve the matview by name).
-- ============================================================
...
-- Rebuild the variant-aware card feed on top of the recreated base view.
...
COMMENT ON MATERIALIZED VIEW public_marketplace_cards IS
  ...
  'Variant children are already excluded by the base view; the master child shows variant_master_title.';
