...
import { buildBrandContext, buildBrandRouterContext } from './build-brand-context.ts';
...
export async function handleBrandTurn(
    ...
    try {
      ...
      if (deterministic.committed) {
        ...
        await addMessage(input.chatId, 'assistant', deterministic.text, undefined, {
          ...
          participantNames: input.participantNames,
          service: input.service,
          metadata: { tools_used: [{ tool: 'brand_booking_create', detail: 'deterministic_commit' }] },
        });
        console.warn(
  ...
  // got it set" style reply but never actually ran brand_booking_create, auto-
  // commit the workorder server-side (when the draft is complete) or override
  // the reply so we don't mislead the customer.
  let guardedText = cleanText;
  let guardedBookingState = postToolBookingState;
  if (!promptContext.isInternal && promptContext.lightspeedSettings.booking.enabled) {
    try {
      ...
      if (guardResult.overrideReason) {
        ...
        if (guardResult.overrideReason === 'auto_committed') {
          guardedBookingState = null;
        }
      }
    } catch (err) {
