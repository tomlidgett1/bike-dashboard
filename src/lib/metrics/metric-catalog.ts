...
export const METRIC_CATALOG: MetricDefinition[] = [
  {
    ...
    label: "Net Sales",
    owner: "Finance",
    description: "Sum of sale line totals after discounts. Primary revenue metric for POS sales.",
    ...
    validDimensions: ["category", "employee_name", "customer_full_name", "brand_name", "supplier_name"],
    caveats: ["Includes discounted lines. Does not include cancelled/refunded-only tickets unless present in mirror."],
    ...
    commonMisinterpretations: [
      "Not the same as gross sales before discounts.",
  ...
  },
  {
    ...
    label: "Gross Sales",
    owner: "Finance",
    description: "Sum of retail/subtotal before discount leakage on sale lines.",
    source: "lightspeed_sales",
    grain: "sale_line_id",
    sqlExpression: "SUM(COALESCE(subtotal, retail * quantity, 0))",
    dateField: "complete_time",
    timezone: STORE_TIMEZONE,
    defaultFilters: [],
    validDimensions: ["category", "employee_name", "customer_full_name"],
    caveats: ["Can exceed net sales when discounts are heavy."],
    freshnessSlaHours: 24,
    sensitivity: "finance",
  ...
  },
  {
    ...
    source: "lightspeed_sales",
    grain: "sale_line_id",
    sqlExpression: "SUM(COALESCE(profit, total - COALESCE(cost, 0)))",
    dateField: "complete_time",
    timezone: STORE_TIMEZONE,
    defaultFilters: ["cost IS NOT NULL OR profit IS NOT NULL"],
    validDimensions: ["category", "employee_name", "brand_name", "supplier_name"],
  ...
  },
  {
    ...
    grain: "sale_line_id",
    sqlExpression:
      "CASE WHEN SUM(total) = 0 THEN NULL ELSE 100.0 * SUM(COALESCE(profit, total - COALESCE(cost, 0))) / NULLIF(SUM(total), 0) END",
    dateField: "complete_time",
    timezone: STORE_TIMEZONE,
    defaultFilters: [],
    validDimensions: ["category", "brand_name", "supplier_name"],
  ...
  },
  {
    ...
    owner: "Finance",
    description: "Net sales divided by distinct sale tickets.",
    source: "lightspeed_sales",
    grain: "sale_id",
  ...
  },
  {
    ...
    timezone: STORE_TIMEZONE,
    defaultFilters: [],
    validDimensions: ["category", "description", "sku", "brand_name"],
    caveats: [],
    freshnessSlaHours: 24,
    sensitivity: "internal",
