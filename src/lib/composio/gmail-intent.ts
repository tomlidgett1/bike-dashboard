...
/**
 * Any Gmail/inbox task — search, summarise, send, draft, connect, rep/contact research.
...
export function isGmailTaskIntent(message: string): boolean {
  ...
  if (/\b(send|draft|compose|write|reply|forward)\b/.test(lower) && /\b(email|mail|gmail)\b/.test(lower)) {
    return true
  }
