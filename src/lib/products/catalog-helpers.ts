...
export function isLightspeedProduct(p: ProductSourceRow): boolean {
  if (
    ...
    p.listing_source === "supplier_scrape" ||
    p.listing_source === "bike_url_import"
  ) {
    return false;
  }
...
export const LIGHTSPEED_SOURCE_OR_FILTER =
  "listing_source.eq.lightspeed,and(lightspeed_item_id.not.is.null,or(listing_source.is.null,listing_source.not.in.(manual,online_catalog,fesports_scrape,supplier_scrape,bike_url_import)))";

/** PostgREST `.or()` filter — must stay in sync with `isLightspeedProduct`. */
export const MANUAL_SOURCE_OR_FILTER =
  "listing_source.in.(manual,online_catalog,fesports_scrape,supplier_scrape,bike_url_import),and(lightspeed_item_id.is.null,or(listing_source.is.null,listing_source.neq.lightspeed))";
