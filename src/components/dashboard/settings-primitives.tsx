...
//
// One consistent rhythm: a titled card, rows that align label/description on the
// left and the control on the right, and a footer for the save action.
// ─────────────────────────────────────────────────────────────────────────────
...
/** Hairline divider used between SettingsRows (full-bleed inside a card). */
export function SettingsDivider({ className }: { className?: string }) {
  return <div className={cn("-mx-6 my-5 h-px bg-border/60", className)} />;
}

/** Stacked field: label above control. For form-style inputs inside a section. */
