...
export const AuthCard = forwardRef<AuthCardHandle, AuthCardProps>(
  function AuthCard({ className, onAuthenticated }, ref) {
    ...
    useImperativeHandle(ref, () => ({ showSignup, reset }), [showSignup, reset]);

    const completeAuth = (result: AuthSuccess) => {
      if (onAuthenticated) {
    ...
    return (
      <div className={cn("w-full max-w-[420px] rounded-3xl bg-white p-7 shadow-2xl sm:p-9", className)}>
