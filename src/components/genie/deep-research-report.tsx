...
import { Download, FileText, Loader2, Send } from "@/components/layout/app-sidebar/dashboard-icons";
import { SendPdfDialog } from "@/components/genie/send-pdf-dialog";
import { isGeniePdfSendRequest } from "@/lib/genie/pdf-request";
...
export function DeepResearchReport({
  ...
  subtitle?: string;
  footerText?: string;
  /** Pre-fill the Gmail send dialog when the user asked to email the PDF. */
  ...
  const [preparingSendCapture, setPreparingSendCapture] = React.useState(false);
  const rootRef = React.useRef<HTMLDivElement | null>(null);
  const sendCaptureRef = React.useRef<HTMLDivElement | null>(null);
  const autoSendPromptedRef = React.useRef(false);
  ...
  React.useEffect(() => {
    if (!sendToEmail || autoSendPromptedRef.current) return;
    if (sourceQuestion && !isGeniePdfSendRequest(sourceQuestion)) return;
    autoSendPromptedRef.current = true;
    setPreparingSendCapture(true);
    const timer = window.setTimeout(() => setSendDialogOpen(true), 400);
    return () => window.clearTimeout(timer);
  }, [sendToEmail, sourceQuestion]);
  ...
  const renderReportDocument = (printMode = false, captureRef?: React.Ref<HTMLDivElement>) => (
    <div
      ref={captureRef}
  ...
  return (
    <div ref={rootRef} className="w-full">
      ...
      <div className="deep-report-no-print mb-3 flex items-center justify-between gap-3 rounded-md border border-slate-200 bg-white/70 px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={printing}
            className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-800 transition hover:bg-slate-50 disabled:opacity-60"
          >
      ...
      {printing && canUseDOM
        ...
        : null}
      {(preparingSendCapture || sendDialogOpen) && canUseDOM
        ? createPortal(
            <div className="deep-report-pdf-capture pointer-events-none fixed -left-[200vw] top-0 w-[820px] bg-white text-[#1e293b]">
              {renderReportDocument(true, sendCaptureRef)}
            </div>,
            document.body,
          )
        : null}
      <SendPdfDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) setPreparingSendCapture(false);
        }}
        reportElement={sendCaptureRef.current}
        getReportElement={() => sendCaptureRef.current}
        reportTitle={reportTitle}
        defaultRecipientEmail={sendToEmail}
