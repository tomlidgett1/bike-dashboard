...
export type UseTextStreamOptions = {
  ...
  /** When true, growing string updates extend content instead of restarting. */
  live?: boolean
}
...
function useTextStream({
  ...
  }, [speed, mode, live, fadeDuration, segmentDelay, characterChunkSize])
  ...
  const startStreaming = useCallback(() => {
    ...
    if (typeof textStream === "string") {
      sourceTextRef.current = textStream
  ...
  const extendLiveStream = useCallback(
    (text: string) => {
  ...
  useEffect(() => {
    if (
      ...
      textStream.startsWith(sourceTextRef.current) &&
      textStream.length >= sourceTextRef.current.length
    ) {
      if (textStream.length === sourceTextRef.current.length) {
        return
      }
      extendLiveStream(textStream)
      return
    }
  ...
  }, [textStream, startStreaming, extendLiveStream])
...
function ResponseStream({
  ...
  const [committedSegmentCount, setCommittedSegmentCount] = useState(0)
  const previousStreamKeyRef = useRef(
    typeof textStream === "string" ? textStream : "async"
  )
  ...
  useEffect(() => {
    const streamKey = typeof textStream === "string" ? textStream : "async"
    if (
      typeof textStream === "string" &&
      live &&
      streamKey.startsWith(previousStreamKeyRef.current) &&
      streamKey.length >= previousStreamKeyRef.current.length
    ) {
    ...
    previousStreamKeyRef.current = streamKey
    setCommittedSegmentCount(0)
  }, [textStream, live])

  useEffect(() => {
    if (!live) return
    setCommittedSegmentCount((prev) => Math.min(prev, segments.length))
  }, [live, segments.length])

  const handleLastSegmentAnimationEnd = useCallback(() => {
    if (live) {
      setCommittedSegmentCount(segments.length)
    }
    if (animationEndRef.current && isComplete) {
      animationEndRef.current()
    }
  }, [isComplete, live, segments.length])
          ...
          <>
            ...
            <div className="relative">
              {segments.map((segment, idx) => {
                ...
                const isCommitted = live && idx < committedSegmentCount
                const isLastNewSegment =
                  live && idx === segments.length - 1 && !isCommitted
                const isLastSegment = !live && idx === segments.length - 1

                return (
                  <span
                    key={`${segment.text}-${idx}`}
                    className={cn(
                      !isCommitted && "fade-segment",
                    ...
                    style={
                      isCommitted
                        ...
                        : {
                            animationDelay: live
                              ? `${(idx - committedSegmentCount) * getSegmentDelay()}ms`
                              : `${idx * getSegmentDelay()}ms`,
                          }
                    }
                    onAnimationEnd={
                      isLastNewSegment || isLastSegment
