/**
 * Canonical product images for the Image Workbench (approved + pending).
 * GET /api/admin/images/workbench-assets?canonicalProductId=
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("[IMAGE WORKBENCH] Workbench assets error:", error);
    ...
    console.error("[IMAGE WORKBENCH] Workbench assets route error:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to load workbench images" },
      { status: 500 },
    );
  }
}
