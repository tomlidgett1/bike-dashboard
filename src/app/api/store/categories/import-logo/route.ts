/**
 * POST /api/store/categories/import-logo
 * Download a logo from a URL (e.g. Serper result) and store it for a carousel.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json()) as { imageUrl?: string; categoryId?: string };
    ...
    const path = `${user.id}/category-${categoryId}-${timestamp}.${ext}`;
    const adminStorage = createServiceRoleClient().storage;

    const { error: uploadError } = await adminStorage.from('logo').upload(path, uploadBuffer, {
    ...
    if (uploadError) {
      console.error('Carousel logo import upload error:', uploadError);
      return NextResponse.json({ error: `Upload failed: ${uploadError.message}` }, { status: 500 });
    }

    const { data: urlData } = adminStorage.from('logo').getPublicUrl(path);
    ...
    if (updateError) {
      console.error('Carousel logo update error:', updateError);
      return NextResponse.json({ error: 'Failed to save carousel logo' }, { status: 500 });
    }

    return NextResponse.json({ url: urlData.publicUrl });
  } catch (err) {
    console.error('Error in POST /api/store/categories/import-logo:', err);
