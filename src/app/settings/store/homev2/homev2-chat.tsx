...
function isGeniePdfRequest(question: string | undefined): boolean {
  if (!question) return false;
  const text = question.toLowerCase().replace(/\s+/g, " ");
  if (!text.includes("pdf")) return false;

  return (
    /\b(create|generate|make|build|produce|prepare|write|export|download|save)\b.{0,100}\bpdf\b/.test(text) ||
    /\bpdf\b.{0,80}\b(report|document|download|export|file|version|copy)\b/.test(text) ||
